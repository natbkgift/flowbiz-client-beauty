...
3. Integration status ถูก claim เกิน implementation: outbound messaging provider เป็น local delivered mock, Stripe/Omise sync เป็น simulated, Facebook auto-reply เป็น simulated, ad spend sync เป็น mock, webhook signatures ส่วนใหญ่ reject เฉพาะ literal `invalid-secret` ไม่ใช่ HMAC validation จริง.
...
5. Medical Safety Classifier ยังไม่มี gate ที่บังคับใช้ก่อน AI auto-reply หรือ AI-generated outbound. ปัจจุบัน AI Agent ใช้ confidence threshold และ keyword list เท่านั้น และสามารถ mark message เป็น `sent` เมื่อ score >= 0.85.
6. Public blog rendering มี XSS-risk path ที่ควรถูกแก้ใน stabilization ก่อนปล่อย production trust: public `renderContent` render HTML ตรงผ่าน `dangerouslySetInnerHTML` และยอมรับ HTML เดิมถ้าพบ tag บางแบบ.
...
- Incomplete webhook verification: `apps/api/src/modules/integration-gateway/*-handler.js`
- AI auto-send/HITL threshold: `apps/api/src/modules/ai-agent/conversation-service.js:250-268`
- Public blog raw HTML rendering: `apps/web/src/public-app.jsx:400-491`
...
| `POST /ai-agent/inbound` | Any authenticated member | Service account/webhook only, or internal test-only; must not be arbitrary user route in prod | Critical |
...
| `GET /blog/posts`, `/blog/posts/:slug` | Public fallback clinicId or default `1001` | Public published-only; no draft via `status=all` without auth/admin | High |
...
| `GET /forum/topics`, `/forum/topics/:idOrSlug` | Public fallback clinicId/default `1001` | Public active-only; hidden/locked only for moderator | Medium |
...
| `POST /integration/webhooks/wix/:clinicId/:workspaceId` | Public webhook + in-memory rate limit + weak secret check | HMAC/signature validation, replay protection, raw buffer, service actor, audit failures/success | Critical |
| `POST /integration/webhooks/zonepang/:clinicId` | Public webhook + in-memory rate limit + weak secret check | HMAC/signature validation, replay protection, raw buffer, explicit workspace resolution, audit | Critical |
| `POST /integration/webhooks/tiktok/:clinicId` | Public webhook + rate limit + weak signature check + raw buffer | Official signature verification, replay protection, workspace binding, audit | Critical |
| `POST /integration/webhooks/facebook/:clinicId` | Public webhook + rate limit + weak signature check + raw buffer | Official Meta signature validation; no simulated reply in prod; audit and consent | Critical |
...
| Facebook Lead Ads webhook | Parses and creates leads, raw buffer exists | Partial | Official signature verification and replay protection required |
| TikTok Lead Ads webhook | Parses and creates leads, raw buffer exists | Partial | Official signature verification and replay protection required |
| Wix webhook | Parses and creates lead | Partial/mock security | Add raw buffer; real signature validation; explicit workspace/clinic binding |
| Zonepang webhook | Parses/update/create lead | Partial/mock security | Add raw buffer; real signature validation; clear external API contract |
