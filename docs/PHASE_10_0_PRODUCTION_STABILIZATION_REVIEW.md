...
- ปรับ Admin/Public UX เป็นภาษาไทยและ sanitize rich content ก่อน render ด้วย DOMPurify + allowlist URL/tag/attribute
- เพิ่ม CSP nonce/security headers จาก web server และปรับ production web build ให้ minify/ไม่ฝัง inline source map เป็นค่าเริ่มต้น
...
- `npm test` ผ่าน 136/136 หลังเปิด PostgreSQL local ผ่าน Docker และ apply migration 036
- `npm audit --audit-level=moderate` ผ่าน 0 vulnerabilities
- Browser smoke บน `http://127.0.0.1:4173/` และ `/admin` ผ่านทั้ง desktop/mobile: title/`lang="th"` ถูกต้อง, public/admin ไม่ blank, header mobile ไม่ overflow, CSP nonce/security headers ถูกส่งจริง. รอบ web-only smoke มี console warning เฉพาะ API fallback เพราะไม่ได้เปิด API local เพื่อเลี่ยงการ mutate data ระหว่างตรวจ
...
- Public/admin blog/forum rich content rendering now sanitizes through DOMPurify + local URL hardening before `dangerouslySetInnerHTML`; keep XSS regression coverage: `apps/web/src/public-app.jsx`, `apps/web/src/app.jsx`, `tests/admin_ui.test.js`
...
| Nginx/HTTPS config | Config exists in repo; web server now emits CSP and baseline security headers for public/admin HTML; web build defaults to production minify without inline source maps | Infra real if deployed | Verify runtime headers before prod; keep CSP nonce-compatible with inline config and JSON-LD |
